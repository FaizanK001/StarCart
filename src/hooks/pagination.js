...
const usePagination = (initialState) => {
  ...
  const perPage = itemsPerPage ? itemsPerPage : 2;
  const pages = Math.ceil(data?.length / perPage);
  const pagination = [];
  const [currentPage, setCurrentPage] = useState(startFrom <= pages ? startFrom : 1);
  const [slicedData, setSlicedData] = useState([data?.results].slice((currentPage - 1) * perPage, currentPage * perPage));
  ...
  const changePage = (page, e) => {
    ...
    if(page !== currentPage) {
      setCurrentPage(page);
      setSlicedData([data?.results].slice((page - 1) * perPage, page * perPage));
    }
  }
  ...
  const goToPrevPage = (e) => {
    ...
    if(currentPage !== 1) {
      setSlicedData([data?.results].slice((currentPage - 2) * perPage, (currentPage - 1) * perPage));
    }
  }
  ...
  const goToNextPage = (e) => {
    ...
    if(currentPage !== pages) {
      setSlicedData([data?.results].slice(currentPage * perPage, (currentPage + 1) * perPage));
    }
  }
